...
1. 使用 `/root/glibc-all-in-one` 来下载指定版本的libc，不要自己使用curl下载。
2. 在使用逆向 mcp 时，注意修改函数名称、变量名称等。
3. 使用 `patchelf` 和 `lief` 来修改elf，如给其更换libc、添加符号等。
4. 调试时使用 pwndbg 和 tmux 来捕获信息，使用前先阅读对应的 skills。
